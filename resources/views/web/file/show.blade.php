                ...
                <div class="col-xl-12 col-lg-12 text-center"> {{-- Centered content in breadcrumb --}}
                    <div class="breadcrumb-wrap text-center"> {{-- Centered breadcrumb text --}}
                        <div class="breadcrumb-title">
                            <h2 class="digital-library-title mb-2">{{ __('Digital Library') }}</h2> {{-- Added specific class for title --}}
                        </div>
                    </div>
                ...
                <div class="col-lg-4 col-md-6 col-sm-6 col-12"> {{-- Added col-sm-6 and col-12 for better responsiveness --}}
                    <div class="material-card shadow-sm p-4 bg-white rounded"> {{-- Increased card padding slightly for inner content --}}
                        <a 
                            ...
                            @if ($material->type === 'PDF' && !$material->is_downloadable) target="_blank" @endif {{-- Only open in new tab if it's a PDF and not downloadable --}}
                        ...
                        <div class="material-title text-center mt-3"> {{-- Adjusted spacing --}}
                            <h5 class="fw-bold mb-1">{{ $material->title }}</h5> {{-- Bolder title, reduced margin --}}
                        </div>

                        <div class="meta-info text-center mt-3">
                            <ul class="list-unstyled mb-0"> {{-- Removed default bottom margin --}}
                                <li class="meta-item">
                                    <i class="far fa-calendar-alt me-1"></i> {{ date('d F, Y', strtotime($material->created_at)) }} {{-- Using 'far' for font-awesome regular --}}
                                </li>
                                <li class="meta-item mt-2"> {{-- Added small top margin for badge --}}
                                    @if ($material->is_public)
                                        <span class="badge bg-success badge-public">{{ __('Public') }}</span>
                                    @else
                                        <span class="badge bg-warning badge-private">{{ __('Private') }}</span>
                                    @endif
                                </li>
                            </ul>
                        </div>
