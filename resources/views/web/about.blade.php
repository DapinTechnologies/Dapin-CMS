...
<main>
    ...
    <h2 class="fw-bold mb-4 text-dark">{{ $about->title }}</h2>
    

<p class="lead mb-4 text-dark">
    {{ htmlspecialchars_decode(strip_tags($about->short_desc, '<p><br><strong><em>')) }}
</p>
    <div class="mb-4 text-dark" style="color: #212529 !important;">{!! $about->description !!}</div>
</div>
...
<section class="values-section py-5 bg-light">
    <div class="container">
        ...
        <div class="row g-4">
            @foreach(App\Models\CoreValue::all() as $value)
            <div class="col-md-3 col-sm-6">
                <div class="value-card text-center p-4 h-100 border-0 shadow-sm bg-white">
                    <div class="value-icon mb-3">
                        <i class="bi bi-{{ $value->icon }} fs-1 text-dark"></i>
                    </div>
                    <h4 class="mb-3 text-dark">{{ $value->title }}</h4>
                    <p class="mb-0 text-dark">{{ $value->description }}</p>
                </div>
            </div>
            @endforeach
        </div>
...
<div class="row my-5 py-4">
    <div class="col-lg-5">
        ...
        @if($director && $director->image)
             <img src="{{ url($director->image) }}" alt="Founder {{ $director->name }}" class="img-fluid rounded shadow-lg w-100">
        @else
            <img src="{{ asset('assets/images/founder.jpg') }}" alt="Founder {{ $director->name }}" class="img-fluid rounded shadow-lg w-100">
        @endif
    </div>
    <div class="col-lg-7 mt-4 mt-lg-0">
        ...
        <div class="message-content">
            ...
            @if($director)
                <p class="text-dark">{{ $director->message }}</p>
            @else
                <p class="text-dark">No message available from the founder.</p>
            @endif
        </div>
        <div class="signature mt-4">
            <p class="mb-1 fw-bold text-dark">{{ $director->name ?? 'Founder' }}</p>
            <p class="text-muted">{{ $director->title ?? 'Director' }}</p>
        </div>
    </div>
</div>
        ...
        <div class="partner-grid">
            ...
            <div class="partner-card">
                <div class="partner-logo-container">
                    @if($partner->logo)
                        <img src="{{ asset('uploads/about-us/partners/' . $partner->logo) }}" 
                             alt="{{ $partner->name }}" 
                             class="partner-logo"
                             loading="lazy">
                    @else
                        <img src="{{ asset('assets/images/default-partner-logo.png') }}" 
                             alt="Default Logo" 
                             class="partner-logo"
                             loading="lazy">
                    @endif
                ...
                <div class="partner-content">
                    ...
                    <p class="partner-description">{{ \Illuminate\Support\Str::limit($partner->description, 120) }}</p>
                  <a href="{{ Str::startsWith($partner->url, 'http') ? $partner->url : 'https://'.$partner->url }}" 
   target="_blank" 
   class="partner-link">
   Visit Website
</a>
        ...
        <div class="accreditation-grid">
            ...
            <div class="accreditation-card">
                ...
                <div class="accreditation-content">
                    <h3 class="accreditation-name">{{ $accreditation->name }}</h3>
                    <p class="accreditation-description">
                        {{ \Illuminate\Support\Str::limit($accreditation->description, 20) }}
                    </p>
                    @if($accreditation->url)
                    <a href="{{ Str::startsWith($accreditation->url, 'http') ? $accreditation->url : 'https://'.$accreditation->url }}" 
                       target="_blank" 
        ...
        <div class="timeline-progress">
            ...
            @foreach($histories as $history)
                <div class="milestone">
                    ...
                    <div class="milestone-year">{{ $history->year }}</div>
                ...
                <div class="testimonial-card p-4 h-100">
                    ...
                    <div class="d-flex align-items-center">
                        <div class="flex-shrink-0">
                            <img src="{{ asset('uploads/testimonial/' . ($testimonial->attach ?? 'assets/images/default-avatar.jpg')) }}" 
