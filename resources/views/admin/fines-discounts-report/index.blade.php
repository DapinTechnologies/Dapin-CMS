                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" id="adjustment_type" name="adjustment_type">
                                            ...
                                            <option value="discount" {{ request('adjustment_type') == 'discount' ? 'selected' : '' }}>Discounts</option>
                                            <option value="fine" {{ request('adjustment_type') == 'fine' ? 'selected' : '' }}>Fines</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="totals">
                            <strong>Total Discounts: KSh {{ number_format($invoices->sum('discount_amount'), 2) }}</strong> | 
                            <strong>Total Fines: KSh {{ number_format($invoices->sum('fine_amount'), 2) }}</strong>
                        </div>
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $invoice->faculty_title }}</td>
                                        <td>{{ $invoice->program_title }}</td>
                                        <td>{{ $invoice->semester_title }}</td>
                                        <td>KSh {{ number_format($invoice->total_fee, 2) }}</td>
                                        <td class="{{ $invoice->discount_amount > 0 ? 'text-success' : '' }}">
                                            KSh {{ number_format($invoice->discount_amount, 2) }}
                                        </td>
                                        <td class="{{ $invoice->fine_amount > 0 ? 'text-danger' : '' }}">
                                            KSh {{ number_format($invoice->fine_amount, 2) }}
                                        </td>
                                        <td>
                                            @if($invoice->adjustment_type)
                                                <span class="badge bg-{{ $invoice->adjustment_type == 'discount' ? 'success' : 'danger' }}">
                                                    {{ ucfirst($invoice->adjustment_type) }}
                                                </span>
                                            @else
                                                N/A
                                            @endif
                                        </td>
                                        <td>{{ $invoice->adjustment_notes ?? 'N/A' }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(dvfCtx, {
        type: 'bar',
        data: {
            labels: @json($discountsVsFines->pluck('type')),
            datasets: [{
                label: 'Amount (KSh)',
                data: @json($discountsVsFines->pluck('total_amount')),
                backgroundColor: [
                    '#1de9b6',
                    '#f44236'
                ],
    ...
    new Chart(dbfCtx, {
        type: 'pie',
        data: {
            labels: @json($discountsByFaculty->pluck('faculty')),
            datasets: [{
                data: @json($discountsByFaculty->pluck('total_amount')),
                backgroundColor: [
                    '#1de9b6',
                    '#04a9f5',
                    '#a389d4',
    ...
    new Chart(fbfCtx, {
        type: 'pie',
        data: {
            labels: @json($finesByFaculty->pluck('faculty')),
            datasets: [{
                data: @json($finesByFaculty->pluck('total_amount')),
                backgroundColor: [
                    '#f44236',
                    '#f4c22b',
                    '#a389d4',
