...
<div class="container-fluid">
    <div class="row mb-4">
        ...
        <div class="col-md-6 text-right">
            ...
            <a href="{{ route($route.'.edit', $feeStructure->id) }}" class="btn btn-primary">
                        ...
                        <div class="col-md-6">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    ...
                                    <span class="info-box-number"> {{ $feeStructure->faculty->title ?? 'N/A' }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    ...
                                    <span class="info-box-number"> {{ $feeStructure->program->title ?? 'N/A' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    ...
                                    <span class="info-box-number"> {{ $feeStructure->semester }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
    <div class="info-box">
        <span>
            <i class="fas fa-{{ $feeStructure->is_active ? 'check' : 'times' }}"></i>
        </span>
        <div class="info-box-content">
            <span class="info-box-text">STATUS:</span>
            <span class="info-box-number {{ $feeStructure->is_active ? 'text-success' : 'text-danger' }}">
                        ...
                        <div class="col-md-4">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    ...
                                    <span class="info-box-number"> Ksh {{ number_format($feeStructure->items->sum('amount'), 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @foreach($feeStructure->items as $index => $item)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>
                                            <strong>{{ $item->fee_category_title }}</strong>
                                            
                                        </td>
                                        <td>{{ number_format($item->amount, 2) }}</td>
                                        <td>
                                            <span class="badge badge-{{ $item->is_one_time ? 'info' : 'warning' }}">
                                                {{ $item->is_one_time ? 'One Time' : 'Recurring' }}
                                            </span>
                                        </td>
                                        <td>
                                            @can('manage fee items')
                                ...
                                <tr class="bg-light">
                                    ...
                                    <td><strong>Ksh {{ number_format($feeStructure->items->sum('amount'), 2) }}</strong></td>
                                    <td colspan="2"></td>
                                </tr>
                            </tbody>
                        </table>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="small-box bg-gradient-success mb-3">
                        <div class="inner">
                            <h3>{{ $studentCount }}</h3>
                            <p>Students in {{ $feeStructure->program->title ?? 'N/A' }}/{{ $feeStructure->semester ?? 'N/A' }}</p>
                        </div>
                        <div class="icon">
    ...
    <div class="card-body">
        <div class="callout callout-info mb-3">
            ...
            <p>{{ $smsCount }} messages sent to students</p>
            <small class="text-muted">Last sent: {{ $lastSmsDate ? $lastSmsDate->format('d M Y H:i') : 'Never' }}</small>
            @if($invoiceCount > 0)
            <div class="mt-2">
                <button class="btn btn-sm btn-outline-primary send-bulk-sms" data-bs-toggle="modal" data-bs-target="#smsConfirmationModal" data-structure="{{ $feeStructure->id }}">
        ...
        <div class="callout callout-success">
            ...
            <p>{{ $emailCount }} emails sent to students</p>
            <small class="text-muted">Last sent: {{ $lastEmailDate ? $lastEmailDate->format('d M Y H:i') : 'Never' }}</small>
            @if($invoiceCount > 0)
            <div class="mt-2">
                <button class="btn btn-sm btn-outline-success send-bulk-email" data-bs-toggle="modal" data-bs-target="#emailConfirmationModal" data-structure="{{ $feeStructure->id }}">
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge badge-primary badge-pill">
                                {{ $firstInvoiceDate ? $firstInvoiceDate->format('d M Y, H:i') : 'N/A' }}
                            </span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Last Invoice Created
                            <span class="badge badge-primary badge-pill">
                                {{ $lastInvoiceDate ? $lastInvoiceDate->format('d M Y, H:i') : 'N/A' }}
                            </span>
                        </li>
                        @endif
                    </ul>
...
<div class="mt-3 text-center">
    ...
    <button type="button" class="btn btn-danger btn-block" data-bs-toggle="modal" data-bs-target="#deleteFeeStructureModal-{{ $feeStructure->id }}">
        <i class="fas fa-trash"></i> Delete This Fee Structure
    </button>
</div>

<!-- Modal -->
<div class="modal fade" id="deleteFeeStructureModal-{{ $feeStructure->id }}" tabindex="-1" aria-labelledby="deleteFeeStructureModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <form action="{{ route($route.'.destroy', $feeStructure->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="deleteReason-{{ $feeStructure->id }}" class="form-label">
                            <i class="fas fa-comment-dots me-1"></i> Reason for deletion <span class="text-danger">*</span>
                        </label>
                        <textarea 
                            class="form-control" 
                            id="deleteReason-{{ $feeStructure->id }}" 
                            name="delete_reason" 
                            rows="3" 
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="studentsModalLabel">Students in {{ $feeStructure->program->title ?? 'N/A' }}/{{ $feeStructure->semester ?? 'N/A' }}</h5>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <a href="{{ route('fees.invoice.show', $invoice->id) }}" target="_blank">
                                        {{ $invoice->invoice_no }}
                                    </a>
                                </td>
                                <td>
                                ...
                                <td>Ksh {{ number_format($invoice->total_fee - $invoice->amount_paid, 2) }}</td>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>Ksh {{ number_format($invoice->total_fee, 2) }}</td>
                                <td>Ksh {{ number_format($invoice->amount_paid, 2) }}</td>
                                <td>{{ $invoice->payment_date ? $invoice->payment_date->format('d M Y, H:i') : 'N/A' }}</td>
                                <td>{{ $invoice->created_at->format('d M Y, H:i') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            @foreach($partialInvoices as $index => $invoice)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <a href="{{ route('fees.invoice.show', $invoice->id) }}" target="_blank">
...
<div class="modal fade" id="pendingInvoicesModal" tabindex="-1" aria-labelledby="pendingInvoicesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($invoice->due_date)
                                        <span class="badge bg-info">
                                            Due in {{ now()->diffInDays($invoice->due_date) }} days
                                        </span>
                                    @else
                                        N/A
                                    @endif
                                ...
                                <td>{{ $invoice->created_at->format('d M Y, H:i') }}</td>
                                <td>
                                    <button class="btn btn-sm btn-warning send-reminder" data-invoice="{{ $invoice->id }}" title="Send Reminder">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary send-bulk-reminders" data-structure="{{ $feeStructure->id }}">
...
$(document).ready(function() {
    ...
    $('.remove-item').click(function() {
        if(confirm('Are you sure you want to remove this fee item?')) {
            ...
            $.ajax({
                url: '{{ route("admin.fee-structures.remove-item") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    id: itemId
