...
@endphp
   ...
   {{-- <a href="{{ route('invoice.show', $invoice->id) }}" 
    ...
    data-student-name="{{ $invoice->studentEnroll->student->full_name ?? 'Student' }}"
    data-amount-due="{{ $invoice->amount_due }}"
    data-due-date="{{ \Carbon\Carbon::parse($invoice->due_date)->format('d M Y') }}"
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="categories[]" id="edit_categories" class="form-control select2" multiple="multiple" required>
                            @foreach($allCategories as $category)
                                <option value="{{ $category->id }}">{{ $category->title }} ({{ number_format($category->amount, 2) }})</option>
                            @endforeach
                        </select>
                    </div>
        ...
        button.addEventListener('click', function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    _token: '{{ csrf_token() }}' // Still include CSRF for security
                },
