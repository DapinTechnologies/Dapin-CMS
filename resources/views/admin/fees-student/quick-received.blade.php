                        ...
                        <div class="card-header-right">
                            <div class="btn-group card-option">
                                ...
                                <ul class="list-unstyled card-option dropdown-menu dropdown-menu-end">
                                    <li><a href="{{ route($route.'.index') }}" class="dropdown-item"><i class="fas fa-list"></i> View Payment History</a></li>
                      ...
                      <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control select2" name="student" id="student" required>
                                ...
                                <option value="{{ $student->id }}" data-balance="{{ $student->outstanding_balance ?? 0 }}" @if(old('student') == $student->id) selected @endif>
                                    {{ $student->student->student_id ?? '' }} - {{ $student->student->first_name ?? '' }} {{ $student->student->last_name ?? '' }}
                                    @if(isset($student->outstanding_balance)) (Balance: {{ $setting->currency_symbol }}{{ number_format($student->outstanding_balance, 2) }}) @endif
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6">
                            <label for="invoice">{{ __('Select Invoice') }}</label>
                            <select class="form-control select2" name="invoice" id="invoice">
                                <option value="">{{ __('select') }}</option>
                                @if(isset($invoices))
                                    @foreach($invoices as $invoice)
                                        <option value="{{ $invoice->id }}" data-amount="{{ $invoice->amount }}" data-balance="{{ $invoice->balance }}">
                                            INV-{{ $invoice->id }} ({{ $setting->currency_symbol }}{{ number_format($invoice->amount, 2) }})
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="form-group col-md-4">
                            <label for="discount_amount" class="form-label">Waiver ({!! $setting->currency_symbol !!}) <span>*</span></label>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="payment_method" id="payment_method" required onchange="togglePaymentFields()">
                                ...
                                <option value="6" @if( old('payment_method') == 6 ) selected @endif>Bursary</option>
                                <option value="7" @if( old('payment_method') == 7 ) selected @endif>M-Pesa</option>
                                <option value="8" @if( old('payment_method') == 8 ) selected @endif>Donation</option>
                            </select>
                            <div class="invalid-feedback">
                        ...
                        <div class="form-group col-md-6" id="bursaryFields" style="display:none;">
                            ...
                            <select class="form-control" name="bursary_id" id="bursary_id" onchange="updateBursaryBalance()">
                                ...
                                @foreach($bursaries as $bursary)
                                    <option value="{{ $bursary->id }}" data-balance="{{ $bursary->remaining_amount }}">
                                        {{ $bursary->name }} (Balance: {{ $setting->currency_symbol }}{{ number_format($bursary->remaining_amount, 2) }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            <div class="alert alert-info">
                                ...
                                <div id="batchSummary">
                                    Number of Students: 0 | Total Amount: {{ $setting->currency_symbol }}0.00
                                </div>
                            </div>
                        </div>
                        <div class="col-md-12 mb-3">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="date" class="form-control" id="recon_start_date" value="{{ date('Y-m-d', strtotime('-7 days')) }}">
                    </div>
                    <div class="col-md-4">
                        <label for="recon_end_date" class="form-label">End Date</label>
                        <input type="date" class="form-control" id="recon_end_date" value="{{ date('Y-m-d') }}">
                    </div>
...
<script type="text/javascript">
            ...
            if(studentId) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        invoiceSelect.empty().append('<option value="">{{ __('select') }}</option>');
    ...
    function validateBursaryBalance(amount) {
        ...
        if(amount > bursaryBalance) {
            $("#bursaryBalance").html('<span class="text-danger">Insufficient bursary funds. Required: {{ $setting->currency_symbol }}' + amount.toFixed(2) + ', Available: {{ $setting->currency_symbol }}' + bursaryBalance.toFixed(2) + '</span>');
            return false;
        } else {
            $("#bursaryBalance").html('Available balance: {{ $setting->currency_symbol }}' + bursaryBalance.toFixed(2));
            return true;
        }
    }
    ...
    function calculateBatchTotal() {
        ...
        $("#batchSummary").html(
            ...
            "Total Amount: {{ $setting->currency_symbol }}" + totalAmount.toFixed(2)
        );
    ...
    function updateBatchBursaryBalance() {
        ...
        if(totalAmount > bursaryBalance) {
            $("#batchBursaryBalance").html(
                '<span class="text-danger">Insufficient bursary funds. Required: {{ $setting->currency_symbol }}' + 
                totalAmount.toFixed(2) + ', Available: {{ $setting->currency_symbol }}' + bursaryBalance.toFixed(2) + '</span>'
            );
            return false;
        } else {
            $("#batchBursaryBalance").html(
                'Available balance: {{ $setting->currency_symbol }}' + bursaryBalance.toFixed(2) + 
                ' | Remaining after allocation: {{ $setting->currency_symbol }}' + (bursaryBalance - totalAmount).toFixed(2)
            );
            return true;
        }
    }
            ...
            success: function(response) {
                ...
                $.each(response.payments, function(index, payment) {
                    ...
                    tbody.append(
                        ...
                        '<td>{{ $setting->currency_symbol }}' + parseFloat(payment.amount).toFixed(2) + '</td>' +
                ...
                $("#reconTotalAmount").text('{{ $setting->currency_symbol }}' + totalAmount.toFixed(2));
            },
            error: function() {
