                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <p class="card-subtitle text-white-80 mb-0">
                                    <i class="fas fa-calendar-alt me-1"></i> Semester: {{ $enroll->semester->title ?? 'N/A' }} • {{ date('Y') }}
                                </p>
                            </div>
                            <div class="badge bg-white text-primary fs-6 py-2 px-3">
                                <i class="fas fa-user-graduate me-1"></i> {{ $enroll->program->title ?? '' }}
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="registration-section mb-5">
                            <div class="section-header d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
                                ...
                                <div class="bg-light p-2 rounded">
                                    ...
                                    <strong>Max Credit Hours:</strong> {{ $enroll->program->max_credit_hour ?? '--' }}
                                </div>
                            </div>
                            
                            @if(isset($available_subjects) && count($available_subjects) > 0)
                            <form class="needs-validation" novalidate action="{{ route($route.'.store') }}" method="post">
                                @csrf
                                <input type="hidden" name="student_enroll_id" value="{{ $enroll->id }}">
                                    ...
                                    <div class="col-md-6 col-lg-4">
                                        <div class="card subject-card h-100 border">
                                            <div class="card-header bg-light py-2">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <span class="badge {{ $subject->subject_type == 0 ? 'bg-primary' : 'bg-success' }} rounded-pill">
                                                        {{ $subject->subject_type == 0 ? __('CORE') : __('Active Now') }}
                                                    </span>
                                                    
                                                </div>
                                            </div>
                                            <div class="card-body">
                                                <h5 class="subject-code text-dark mb-1">Unit Code: {{ $subject->code }}</h5>
                                                <h4 class="subject-title text-dark mb-2">{{ $subject->title }}</h4>
                                                
                                                <div class="subject-meta mb-3">
                                                    <div class="d-flex align-items-center mb-1">
                                                        <i class="fas fa-user-tie text-muted me-2"></i>
                                                        <small class="text-muted">Lecturer: {{ $subject->teacher->first_name ?? 'TBA' }}</small>
                                                    </div>
                                                ...
                                                <div class="subject-description">
                                                    <p class="text-muted small mb-0">{{ Str::limit($subject->description, 100) }}</p>
                                                </div>
                                            </div>
                                            <div class="card-footer bg-transparent border-top-0 pt-0">
                                                <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#courseModal-{{ $subject->id }}">
